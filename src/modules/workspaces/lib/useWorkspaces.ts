...
export function panelInsertionIndex(
  panels: Panel[],
  activePanelId: string | null,
  insertionIndex?: number,
): number {
  if (insertionIndex !== undefined) return Math.min(insertionIndex, panels.length);
  const activeIdx = panels.findIndex((p) => p.id === activePanelId);
  return activeIdx >= 0 ? activeIdx + 1 : panels.length;
}
      ...
      prev.map((w) => {
        ...
        return {
          ...
          paneTree: updatePane(w.paneTree, paneId, (p) => {
            const idx = panelInsertionIndex(p.panels, p.activePanelId, insertionIndex);
