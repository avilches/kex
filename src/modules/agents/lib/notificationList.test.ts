...
describe("buildAgentEntries", () => {
  ...
  test("el dot naranja (attention) se deriva de la sesion viva, no de la notificacion", () => {
    const sessions = {
      p1: session({ tabId: "p1", status: "attention", attentionSince: 100 }),
    };
    const notifs = [notif({ tabId: "p1", kind: "error", at: 50 })];
    const entries = buildAgentEntries(sessions, notifs);
    expect(entries).toHaveLength(1);
    expect(entries[0].visual).toBe("attention");
    expect(entries[0].pending).toBe(true);
  });

  test("orden: attention primero, luego working, luego error; por tiempo desc dentro del grupo", () => {
    const sessions = {
      w1: session({ tabId: "w1", status: "attention", attentionSince: 10 }),
      w2: session({ tabId: "w2", status: "attention", attentionSince: 20 }),
