...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S kex-bin";
    case "debian":
      return `sudo apt install ./Kex_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Kex-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Kex v${manual.version} is available`
                  : `Kex v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Kex to finish installing."
              : downloading
                ? progress !== null
