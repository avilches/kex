...
const FISH_REINSTALL_PROMPT: &str =
    "functions -q __kex_install_prompt; and __kex_install_prompt";
...
pub fn build_command(
    ...
    {
        let _ = workspace;
        let mut cmd = unix::build(cwd, blocks, tab_id.as_deref(), ipc_path, shell)?;
        cmd.env("TERMINAL_ID", id.to_string());
        Ok(cmd)
    }
    #[cfg(windows)]
    {
        let mut cmd = windows::build(cwd, workspace, blocks, tab_id.as_deref(), ipc_path, shell)?;
        cmd.env("TERMINAL_ID", id.to_string());
        Ok(cmd)
    }
}
...
mod unix {
    ...
    pub fn build(cwd: Option<String>, blocks: bool, tab_id: Option<&str>, ipc_path: Option<&str>, shell_override: Option<String>) -> Result<CommandBuilder, String> {
        let (shell, shell_path) = Shell::resolve(shell_override);
        let mut cmd = CommandBuilder::new(&shell_path);
        super::apply_common(&mut cmd, cwd, blocks, tab_id, ipc_path);

        match shell {
            Shell::Zsh => {
                match prepare_zdotdir() {
                    Ok(zdotdir) => {
                        // Guard against Kex-in-Kex :)
                        if let Ok(user_zd) = std::env::var("ZDOTDIR") {
                            if Path::new(&user_zd) != zdotdir.as_path() {
                                cmd.env("KEX_USER_ZDOTDIR", user_zd);
                            }
                        }
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("kex").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("kex.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("KEX_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/kex/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("kex.fish");
