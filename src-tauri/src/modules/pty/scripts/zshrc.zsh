...
# zsh, so we shadow $? into `_kex_ret`.

{
  _kex_user_zdotdir="${KEX_USER_ZDOTDIR:-$HOME}"
  [ -f "$_kex_user_zdotdir/.zshrc" ] && source "$_kex_user_zdotdir/.zshrc"
...
if [[ -z "$__KEX_HOOKS_LOADED" ]]; then
  __KEX_HOOKS_LOADED=1
  ...
  _kex_precmd() {
    local _kex_ret=$?
    printf '\e]133;D;%s\e\\' "$_kex_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_kex_urlencode "$PWD")"
  ...
  _kex_preexec() {
    ...
    [[ -n "$KEX_BLOCKS" ]] && _kex_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _kex_precmd
    add-zsh-hook preexec _kex_preexec
  fi
