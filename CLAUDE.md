...
| **MilkdownTab / MilkdownEditor** | Segundo editor WYSIWYG de markdown, en evaluacion, sobre `@milkdown/crepe` (`markdownEngine: "milkdown"`). Mismo shell de Rich/Source y outline que TipTap; sin mermaid (el plugin de diagramas no dibuja nada, ver `docs/FORK.md`) y sin resolucion de imagenes locales via `convertFileSrc` (gap sin verificar en una app real, ver `docs/MILKDOWN_CHECKLIST.md`) | `src/modules/markdown/milkdown/` (shell `MilkdownTab.tsx`, editor `MilkdownEditor.tsx`, outline en `outline.ts` + `OutlinePanel.tsx`) | "el milkdown", "el otro editor", "crepe", "el editor nuevo de markdown" |
