...
Scope note: the focused-transition effect that drives most of this invariant is gated on `!blocks`
(`if (gained && !blocks) requestLeafFocus(leafId);`); block-mode terminals have their own focus
handling, out of this doc's scope.
...
| Open folder in terminal | `src/app/App.tsx` : `openFolderInTerminal` (`setTimeout(() => requestLeafFocus(tabId), 80)` after creating the tab, so the leaf exists by the time focus is requested) |
| New workspace from folder | `src/app/App.tsx` : `newWorkspaceFromFolder` (same `setTimeout(..., 80)` pattern, on the fresh workspace's first tab) |
| Insert a history command into the session | `src/app/App.tsx` : `insertHistoryCommand` (writes the command, then `requestLeafFocus(activeTabId)`) |
| Explorer inline-search close | `src/app/App.tsx` : `onExplorerSearchClose` (calls `searchTarget.focus()`, whose terminal-kind branch is `requestLeafFocus(activeTabId)`) |
| Slot unhide after rebind or GPU retry | `src/modules/terminal/lib/rendererPool.ts` : `scheduleUnhide` calls `adapter.focusLeaf(leafId)`, which is `requestLeafFocus(leafId)` (`useTerminalSession.ts`'s adapter), only when the unhidden leaf is already the focused one |
...
- **Textarea blur**: `ScratchpadBar.tsx`'s `onBlur` handler has two real guards, plus a separate
  mechanism that keeps blur from firing at all on a frame click:
  - `if (settingsOpen) return;` guards losing focus to the scratchpad's own settings dropdown (Radix,
    portaled to `document.body`), which is not a real "leave". `settingsOpen` (React state mirroring
    the `DropdownMenu`'s `open`) is the guard, **not** DOM containment: Radix content lives outside
    `containerRef`, so an `e.relatedTarget` containment check can't see it. `onCloseAutoFocus` on the
    dropdown content brings focus back to the textarea when it closes.
  - `if (next && containerRef.current?.contains(next)) return;` guards a focus move that stays within
    the bar itself (e.g. Tab to the settings button), which is also not a "leave".
  - Otherwise: `leaveLeafScratchpad(leafId)`, a `"leave"` close.

  Separately, the container's `onMouseDown` calls `e.preventDefault()` when the click target is inside
  `containerRef` but is not the textarea itself (i.e. a click on the bar's own padding/hint area). This
  prevents the blur from firing in the first place for that click, rather than guarding inside the
...
`runWorkspaceConfig` and `stopWorkspaceConfig` (`src/app/App.tsx`) are symmetric: both implement the
same rule, and the same three cases, for run and stop alike. The rule: **a pane shows exactly one tab,
so you cannot reveal a tab in the user's own pane without also stealing their focus.** This drives
three cases:
...
- **Radix portals break DOM-containment guards.** The `onBlur` handler's
  `containerRef.current?.contains(relatedTarget)` check correctly covers focus moves that stay within
  the bar (see section 4), but it cannot see Radix dropdown/menu content, because Radix portals it to
  `document.body`. Use the menu's own `open` state as the guard for that case instead (see
...
| `src/app/App.tsx` | `restoreLeafFocus`, `leaveActivePaneScratchpad`, `focusActiveTab`, the workspace-switch effect (`prevWorkspaceIdRef`), the sentinel effect (`prevActiveLeafRef`), `onFocusChanged`, the bell-close effect, `onActivateTabStable`, `onActivateAgent`, `runWorkspaceConfig`/`stopWorkspaceConfig`, `openFolderInTerminal`, `newWorkspaceFromFolder`, `insertHistoryCommand`, `onExplorerSearchClose` |
...
| `src/modules/terminal/lib/rendererPool.ts` | `scheduleUnhide` calls `adapter.focusLeaf` (`requestLeafFocus`) after a slot rebind or GPU retry, only when the leaf is the focused one |
