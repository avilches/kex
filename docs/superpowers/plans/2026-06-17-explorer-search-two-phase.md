...
**Goal:** Show file-search results immediately from a shallow tree scan while the full scan continues in the background, with "Searching…" / "Searching deeper…" labels to communicate progress.

**Architecture:** Two parallel `invoke("fs_search", ...)` calls are launched after the same 300ms debounce. Phase 1 uses `max_depth=3` and resolves in ~5-20ms; its hits are shown immediately and the label switches to "Searching deeper…". Phase 2 uses the default `max_depth=8`; when it resolves, results are replaced while preserving the user's keyboard selection. No Rust changes needed — `max_depth` already exists.

**Tech Stack:** React 19, TypeScript, Tauri `invoke`. Single file change: `src/modules/explorer/ExplorerSearch.tsx`.

## Global Constraints

- No em-dash anywhere (code, comments, labels, commit message).
- No emojis.
- `pnpm lint`, `pnpm check-types`, `pnpm test` must pass.
- No Rust changes.
- Spinner labels must be exactly `"Searching…"` and `"Searching deeper…"` (with ellipsis `…`, not three dots `...`).

---

### Task 1: Two-phase progressive search in ExplorerSearch

**Files:**
- Modify: `src/modules/explorer/ExplorerSearch.tsx` (full rewrite of logic; JSX structure preserved)

**Interfaces:**
- Consumes: `invoke<SearchResult>("fs_search", { root, query, limit, maxDepth, showHidden, workspace })` — `maxDepth` is the new optional param added in the previous optimization; omitting it uses the Rust default of 8.
- Produces: no new exports; `ExplorerSearchHandle` and `ExplorerSearch` signatures unchanged.

- [ ] **Step 1: Read the current file to confirm its state**

```bash
wc -l src/modules/explorer/ExplorerSearch.tsx
```

Expected: 327 lines. If different, read the file before proceeding to understand actual current state.

- [ ] **Step 2: Replace `ExplorerSearch.tsx` with the implementation below**

Write the following complete file to `src/modules/explorer/ExplorerSearch.tsx`:
...
Expected: no output (clean). If there are errors, fix them before continuing.

- [ ] **Step 4: Run lint**

```bash
pnpm lint 2>&1 | grep -v "warning\|Warning" | head -20
```

Expected: the one pre-existing error in `src/lib/pathUtils.ts` is the only output. If new errors appear in `ExplorerSearch.tsx`, fix them.
...
Expected: `Tests 284 passed (284)` (or similar count — all green).

- [ ] **Step 6: Commit**

```bash
git add src/modules/explorer/ExplorerSearch.tsx
git commit -m "feat(search): busqueda en dos fases con resultados progresivos"
```
