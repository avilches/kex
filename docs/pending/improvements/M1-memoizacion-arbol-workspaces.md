...
4. ~~**Estabilizar callbacks**~~ **HECHO** — callbacks de `PaneView` hacia `PaneTabBar` estabilizados con `useCallback`. Los de `App.tsx` hacia `WorkspaceView` y `RightPanel` también estabilizados: `activeCwdRef` + `workspacesRef` usados internamente; `onOpenCommitFile` usa `activeWorkspaceId` (no cambia en `cd`) + ref. Los callbacks ya no se recrean en cada `cd`.
