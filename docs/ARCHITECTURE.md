...
- **File System**: root starts at the user's home directory and is navigable per workspace (`fsRoot` field in `workspace-state.json`, default home). Double-clicking a folder enters it (sets `fsRoot`); an Up button climbs to the parent directory, capped at the filesystem or drive root; a Home button (hidden when already at home) returns to the user's home in one click. These controls exist only in this mode; the other three modes are unaffected and double-click does nothing. The JSON-only preference `keepFolderLayoutOnChangeExplorerRoot` (default `false`) controls whether the per-root expansion layout is restored on a root change or the tree starts collapsed.
