...
- **F4** (`sidebar.focusTerminal`) always anchors: it applies the cascade (see below), opens the right panel if closed, and switches away from Git History to Explorer if needed (it stays on Explorer or Source Control otherwise).
