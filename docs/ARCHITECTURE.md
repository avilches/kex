...
- **Per-terminal autofocus flag** -- toggled from the tab right-click context menu (crosshair icon shown on the tab when active). When a terminal with autofocus gains focus or its cwd changes while it has focus, it anchors silently: the same cascade runs, but the panel is never opened and the active sidebar tab is never changed.
