...
**Scratchpad bar.** A per-terminal input bar (`terminal.scratchpad`, default `Cmd+U`) to compose multi-line text and send it to the shell without typing at the prompt. It supports drag-and-drop of explorer paths (inserted as `@`-prefixed references) and an Enter / Shift+Enter send-mode preference (`scratchpadEnterSends`). Enabled/disabled is a single boolean persisted per terminal panel (`Tab.scratchpadEnabled`) and restored on launch; new terminals open it by default when `scratchpadInNewTerminals` is on. Visibility is derived, not persisted: `enabled && tab is focused`. Whenever a tab with the scratchpad enabled regains focus, keyboard focus always goes to the scratchpad textarea (never a "last active side" memory); clicking inside the terminal grid moves focus there only transiently, for as long as the tab stays focused. The bar can also be toggled from the tab context menu.
