...
A workspace can exist with no open tabs. Closing the last tab in the sole pane of a workspace empties that pane instead of destroying the workspace; the empty pane renders a welcome screen with quick actions (new terminal, open file). A workspace is removed only by an explicit close action (the close shortcut on an empty workspace, or the sidebar close button). The close action runs a two-stage cascade: first, if `warnOnCloseTabWithRunningProcess` is on, it checks every terminal in the workspace for a live foreground process (via `collectRunningTerminals` + `leafHasForegroundProcess`) and, if any are found, shows a dialog listing them with an option to disable the warning; second, if no running processes were found (or the preference is off), and `warnOnCloseWorkspace` is on, it shows a plain confirmation dialog; otherwise it closes directly. Closing the last workspace quits the app. Panes inside a split still collapse when their last tab is closed - only the sole pane of a workspace stays empty.
...
**Sidebar-driving terminal (autofocus).** One terminal at a time drives both the Explorer root and the Source Control / Git History repo. Anchoring happens via two explicit gestures only -- never automatically on every focus change or cwd update:
...
- **Per-terminal autofocus flag** -- toggled from the terminal tab hover card (crosshair indicator on the tab when on). When a terminal with autofocus gains focus or its cwd changes while it has focus, it anchors silently: the same cascade runs, but the panel is never opened and the active sidebar tab is never changed.

The shared cascade (`resolveSidebarTarget`, tested): given the panel's reference path (a terminal's folder or an editor / git-diff file), if a git repo nested **strictly inside** the workspace root owns that path it sets File System rooted at the nested repo (so a worktree or sub-repo under the pinned root is shown, not the parent); else if the path is under the workspace root it sets Workspace Root mode; else if it is under a git repo it sets File System rooted at the git root; else it sets File System rooted at the common ancestor of the current fs root and the path (falling back to `dirname`). The git repo is resolved from the same path via `git_resolve_repo`, which accepts a file path (resolving from its parent directory) so editor tabs anchor too. Source Control and Git History resolve their repo from the resulting explorer root (`useSourceControl`); that resolution is keyed on the exact context path (`canReuseResolvedRepo`, tested), never on a path prefix, so anchoring into a nested repo/worktree re-resolves to it instead of reusing the parent. The `autofocus` flag is persisted per terminal panel in `workspace-state.json`. New-workspace terminals default to no autofocus; when no autofocus terminal exists the sidebar root and git repo stay wherever they were last anchored.
