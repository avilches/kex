...
- PENDIENTE DE DECIDIR (sync sigue abierto): mejoras de UI derivadas de motion tokens (6ebb6b8 panel swap, 731da51 header hover, bb155d2 tab enter; las keyframes terax-panel-in/terax-tab-in estan definidas en globals.css pero sin cablear) y CI menores (c4aaca2 pnpm desde packageManager, afd1167 firma appimage updater en release). El resto del scope aceptado (Bucket B completo, C1/C2/C3/C5) ya estaba aplicado.
- Quality suite (frontend): check-types OK, lint exit 0 (84 warnings preexistentes), 180 tests (+3 hasAnyBlock), vite build OK. Rust no tocado.
