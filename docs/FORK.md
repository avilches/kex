...
- **Two-column layout** — Quick Access (pinned notes, drag-reorder) and a folder tree on the left; a sortable note list on the right, scoped to exactly one folder at a time (its immediate subfolders render as rows on top, always alphabetical, with the count of notes directly inside each). Four sort modes: `modified`, `title`, `created`, `custom`, plus optional date grouping.
